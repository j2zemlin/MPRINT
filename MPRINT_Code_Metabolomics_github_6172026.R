...
# ─────────────────────────────────────────────────────────────
# LOO cross-validation + permutation test (mixOmics)
# ─────────────────────────────────────────────────────────────

# Reconstruct X and Y exactly as used to build PLSDA_cohort
X <- data_final_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE))))
Y <- PCA_whole_scores$ATTRIBUTE_abtreatment

# --- Observed performance: leave-one-out 
PLSDA_cohort_perf_loo <- perf(
  PLSDA_cohort,
  validation = "loo",
  auc = TRUE,
  progressBar = TRUE
...
# Inspect structure once to confirm field names, then extract BER
str(PLSDA_cohort_perf_loo$error.rate)
plot(PLSDA_cohort_perf_loo, col = color.mixo(5:7), legend.position = "horizontal")

# Observed balanced error rate at comp 2, max.dist
observed_BER <- PLSDA_cohort_perf_loo$error.rate$BER[2, "max.dist"]
print(PLSDA_cohort_perf_loo$error.rate$BER)
print(PLSDA_cohort_perf_loo$auc)

# --- Permutation test: shuffle labels, refit, recompute LOO BER ---
set.seed(123)
n_perm   <- 999
...
for (i in seq_len(n_perm)) {
  Y_perm    <- sample(Y)
  mod_perm  <- mixOmics::plsda(X, Y_perm, ncomp = 2, scale = TRUE)
  perf_perm <- perf(mod_perm, validation = "loo")
  perm_BER[i] <- perf_perm$error.rate$BER[2, "max.dist"]
  setTxtProgressBar(pb, i)
}
close(pb)

# --- Empirical p-value ---
perm_p <- (sum(perm_BER <= observed_BER) + 1) / (n_perm + 1)

cat("\nObserved BER (comp 2):", round(observed_BER, 3), "\n")
cat("Permutation p-value:  ", signif(perm_p, 4), "\n")

# --- Null distribution plot ---
hist(perm_BER, breaks = 30, col = "grey85", border = "white",
     main = "Permutation null distribution (LOO BER, comp 2)",
     xlab = "Balanced error rate")
...
## --- 1. Extract component 1 loadings directly from the PLSDA model ----
loadings_comp1 <- PLSDA_cohort$loadings$X[, 1]  # numeric vector

Loadings_cohort <- data.frame(
  ID      = names(loadings_comp1),
  Loading = as.numeric(loadings_comp1),
  row.names = NULL)
...
# 3. Perform M-fold cross-validation to evaluate the PLSDA model
perf_plsda_cohort <- perf(PLSDA_cohort, validation = "Mfold", folds = 4, nrepeat = 99, progressBar = TRUE) 

# 4. Plot and print performance
plot(perf_plsda_cohort, legend = TRUE)
print(perf_plsda_cohort)

# 5. Extract VIPs and filter based on component 1
...
vip_df <- read_csv("VIPs_cohort_Load_comp1.csv") 
